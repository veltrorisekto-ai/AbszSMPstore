...
export default async function handler(req,res){
  try{
    const mc=await getSetting('minecraft');
      ...
      const rows=await sql`
        WITH next_job AS (
          ...
          WHERE attempts < 10 AND (
            (status='QUEUED' AND (last_attempt_at IS NULL OR last_attempt_at < now()-interval '15 seconds'))
            OR
            (status='DELIVERING' AND last_attempt_at < now()-interval '2 minutes')
          )
          ORDER BY created_at
        ...
        SET status='DELIVERING',attempts=d.attempts+1,last_attempt_at=now(),updated_at=now()
        FROM next_job n
        WHERE d.id=n.id
        RETURNING d.*`;
      if(!rows[0])return send(res,200,{ok:true,job:null});
      const j=rows[0];
      await sql`UPDATE orders SET status='DELIVERING',delivery_status='DELIVERING',updated_at=now() WHERE id=${j.order_id} AND delivery_status<>'DELIVERED'`;
    ...
    if(action==='ack'){
      ...
      const key=String(body.delivery_key||'');
      const job=(await sql`SELECT * FROM delivery_jobs WHERE id=${id} AND delivery_key=${key} LIMIT 1`)[0];
      if(!job)return send(res,404,{ok:false,error:'Delivery job not found'});
      if(job.status==='COMPLETED')return send(res,200,{ok:true,idempotent:true});

      if(body.success===true){
        const completed=await sql`UPDATE delivery_jobs SET status='COMPLETED',last_error=NULL,completed_at=COALESCE(completed_at,now()),updated_at=now() WHERE id=${id} AND status<>'COMPLETED' RETURNING *`;
        if(!completed[0])return send(res,200,{ok:true,idempotent:true});
        const order=(await sql`UPDATE orders SET status='DELIVERED',delivery_status='DELIVERED',delivered_at=COALESCE(delivered_at,now()),updated_at=now() WHERE id=${job.order_id} RETURNING *`)[0];
